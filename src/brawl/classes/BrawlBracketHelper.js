const { EmbedBuilder, AttachmentBuilder, ButtonBuilder, ButtonStyle, ActionRowBuilder, ComponentType, ThreadAutoArchiveDuration } = require("discord.js");
...
class Match {
  ...
  async addBonusVotes(userIDs, myUserStat) {
    ...
    for (const reactedUser of userIDs) {
      ...
      if (member.roles.cache.some((role) => role.name === "Owner")) {
        ...
        bonus = Math.max(config.brawl.activeBoosterBonus, config.brawl.serverSubscriberBonus);
      }
      totalCount += bonus;
  ...
  async conductMatch(channel, bracketModel, setupModel, myUserStat) {
    ...
    // Combine card images
    const image1 = readImage(this.card1);
    const image2 = readImage(this.card2);
    ...
    const buttonTotal = new ButtonBuilder().setCustomId("buttonTotal").setLabel("0 Voters").setStyle(ButtonStyle.Secondary);
    ...
    if (count1 > count2) {
      ...
      if (difference === 1) {
        await channel.send(`**Card 1** won by just **1** vote, with ${bonus1} bonus! [**${count1}**:**${count2}**]`);
      } else {
        await channel.send(`**Card 1** won by **${difference}** votes, with ${bonus1} bonus! [**${count1}**:**${count2}**]`);
      }
      ...
      if (difference === 1) {
        await channel.send(`**Card 2** won by just **1** vote, with ${bonus2} bonus! [**${count1}**:**${count2}**]`);
      } else {
        await channel.send(`**Card 2** won by **${difference}** votes, with ${bonus2} bonus! [**${count1}**:**${count2}**]`);
      }
      ...
      await channel.send(`Voting ended in a **tie** with **${count1}** votes each. The lucky winner is... 🥁`).then(async (msg) => {
        ...
        if (this.winner === this.card1) {
          msg.edit(`Voting ended in a **tie** with **${count1}** votes each. The lucky winner is... **Card 1**! 🎉`);
          myUserStat.updateMatchesCompeted(owner1, true, true);
          myUserStat.updateMatchesCompeted(owner2, false, true);
        } else {
          msg.edit(`Voting ended in a **tie** with **${count1}** votes each. The lucky winner is... **Card 2**! 🎉`);
...
class BrawlBracketHelper {
  ...
  generateInitialBracket() {
    ...
    console.log(`[BRAWL BRACKET] ${diff} free pass matches + ${competitorsSize - diff} normal matches = ${competitorsSize} total competitors`);
  ...
  async conductTournament() {
    ...
    while (this.bracketModel.completedMatches.length !== this.idealSize - 1) {
      ...
      const completedMatchSchema = await currentMatch.conductMatch(channel, this.bracketModel, this.setupModel, this.myUserStat);
    ...
    await this.postHallOfFame(this.bracketModel, this.setupModel);
  }
  ...
  async announceMentions(channel) {
    ...
    const leastFile = new AttachmentBuilder(path.join(config.brawl.cards_directory, `${leastVotes.card}.png`));
    const leastImage = `attachment://${leastVotes.card}.png`;
    const leastEmbed = new EmbedBuilder().setTitle("Least Votes").setDescription(`Votes: **${leastVotes.count}**\nCard: \`${leastVotes.card}\` by <@${leastID}>`).setImage(leastImage);
    await channel.send({
      embeds: [leastEmbed],
      files: [leastFile],
    });
    ...
    const mostFile = new AttachmentBuilder(path.join(config.brawl.cards_directory, `${mostVotes.card}.png`));
    const mostImage = `attachment://${mostVotes.card}.png`;
    const mostEmbed = new EmbedBuilder().setTitle("Most Votes").setDescription(`Votes: **${mostVotes.count}**\nCard: \`${mostVotes.card}\` by <@${mostID}>`).setImage(mostImage);
    await channel.send({
      embeds: [mostEmbed],
      files: [mostFile],
    });
  ...
  async announceWinner(channel) {
    const finalsMatch = this.bracketModel.completedMatches[this.bracketModel.completedMatches.length - 1];
    ...
    const semisMatch =
      this.bracketModel.completedMatches[this.bracketModel.completedMatches.length - 2].winner === winnerCard
    ...
    // 3rd place
    const thirdFile = new AttachmentBuilder(path.join(config.brawl.cards_directory, `${thirdCard}.png`));
    const thirdImage = `attachment://${thirdCard}.png`;
    const thirdEmbed = new EmbedBuilder().setTitle("Third Place").setDescription(`Card: \`${thirdCard}\` by <@${thirdID}>`).setImage(thirdImage);
    await channel.send({
      embeds: [thirdEmbed],
      files: [thirdFile],
    });
    ...
    // 2nd place
    const secondFile = new AttachmentBuilder(path.join(config.brawl.cards_directory, `${secondCard}.png`));
    const secondImage = `attachment://${secondCard}.png`;
    const secondEmbed = new EmbedBuilder().setTitle("Second Place").setDescription(`\nCard: \`${secondCard}\` by <@${secondID}>`).setImage(secondImage);
    await channel.send({
      embeds: [secondEmbed],
      files: [secondFile],
    });
    ...
    // Send winner embed
    const winnerFile = new AttachmentBuilder(path.join(config.brawl.cards_directory, `${winnerCard}.png`));
    await channel.send({
    ...
    competitorsChannel.messages.fetch(this.setupModel.messageID).then((message) => {
      ...
      updatedEmbed.setImage(`attachment://${winnerCard}.png`);
  ...
  async postHallOfFame(bracketModel, setupModel) {
    const finalsMatch = bracketModel.completedMatches[bracketModel.completedMatches.length - 1];
    const winnerCard = finalsMatch.winner;

    const guild = client.guilds.cache.get(config.guildID);
    const mediaChannel = guild.channels.cache.get(config.channelID.brawlHallOfFame);
    const attachment = new AttachmentBuilder(path.join(config.brawl.cards_directory, `${winnerCard}.png`));
...
// Helper function to read a file from the specified directory
function readImage(fileName) {
  const directoryPath = config.brawl.cards_directory;
  return fs.readFileSync(path.join(directoryPath, `${fileName}.png`));
}
